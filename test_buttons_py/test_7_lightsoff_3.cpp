...
void update_shuffle_counter()
{
    shuffle_counter += shuffle_increment;
    shuffle_increment = (shuffle_increment > 100) ? shuffle_increment - 1 : 200;
}
...
void shuffle(uint8_t count)
{
    ...
    {
        update_shuffle_counter();
        uint8_t r = (shuffle_counter) % 5; // row 0..4
        update_shuffle_counter();
        uint8_t c = (shuffle_counter) % 5; // col 0..4
        handle_press(r, c);
    }
}
